...
function trackEvent(name,props={}){if(window.AFFDTracking?.trackEvent)return window.AFFDTracking.trackEvent(name,props);const payload={tool:"amazon_flat_file_diagnoser",...props};try{if(window.va?.track)window.va.track(name,payload);if(window.gtag)window.gtag("event",name,payload);window.dataLayer=window.dataLayer||[];window.dataLayer.push({event:name,...payload});}catch(_){}}
function safeTrackedHref(href=""){if(window.AFFDTracking?.safeTrackedHref)return window.AFFDTracking.safeTrackedHref(href);if(!href)return"";if(href.startsWith("mailto:"))return"mailto:private-draft";try{const url=new URL(href,window.location.origin);if(url.origin===window.location.origin)return url.pathname;if(url.hostname==="github.com"&&url.search)return `${url.origin}${url.pathname}?public-safe-query`;return url.origin+url.pathname;}catch(_){return String(href).split("?")[0].slice(0,120);}}
const lazyScripts=new Map();
function loadScriptOnce(src){if(lazyScripts.has(src))return lazyScripts.get(src);const job=new Promise((resolve,reject)=>{const existing=document.querySelector(`script[src="${src}"]`);if(existing?.dataset.loaded==="true"){resolve();return;}if(existing){existing.addEventListener("load",()=>resolve(),{once:true});existing.addEventListener("error",()=>reject(new Error(`Failed to load ${src}`)),{once:true});return;}const script=document.createElement("script");script.src=src;script.defer=true;script.dataset.lazy="true";script.addEventListener("load",()=>{script.dataset.loaded="true";resolve();},{once:true});script.addEventListener("error",()=>reject(new Error(`Failed to load ${src}`)),{once:true});document.head.appendChild(script);});lazyScripts.set(src,job);return job;}
function ensureParserLoaded(){return typeof parseDelimited==="function"&&typeof toErrors==="function"?Promise.resolve():loadScriptOnce("./scripts/parser.js");}
async function ensureVariationToolsLoaded(){await ensureParserLoaded();if(typeof analyzeVariationFlatFile!=="function")await loadScriptOnce("./scripts/variation-analyzer.js");}
function ensureCsvExportLoaded(){return typeof csvCell==="function"?Promise.resolve():loadScriptOnce("./scripts/csv-export.js");}
...
function setActiveView(view){state.currentView=view;els.tabButtons.forEach(b=>{const active=b.dataset.view===view;b.classList.toggle("active",active);b.setAttribute("role","tab");b.setAttribute("aria-selected",String(active));b.setAttribute("tabindex",active?"0":"-1");});}
...
async function readFile(file){const ext=file.name.split(".").pop().toLowerCase();if(!supported.has(ext)){toast("第一版暂不支持该格式，请上传 .txt / .csv / .tsv");return;}if(file.size>2*1024*1024){toast("文件过大，请上传 2MB 以内的 Processing Report");return;}await processText(await decodeFileText(file),file.name);trackEvent("processing_report_uploaded",{file_extension:ext,file_size:file.size});}
...
async function downloadCsv(){if(!state.errors.length)return;await ensureCsvExportLoaded();const rows=[["sku","rowNumber","errorCode","severity","attribute","message","recommendedFields","ruleSource","confidence"],...state.errors.map(i=>{const r=rule(i.errorCode);return[i.sku,i.rowNumber,i.errorCode,sevLabel(r.severity),i.attribute,i.message,r.likelyFields.join(" | "),r.sourceLabel,confidenceLabel(r.confidence)];})];const csv=rows.map(row=>row.map(csvCell).join(",")).join("\n"),url=URL.createObjectURL(new Blob([`\uFEFF${csv}`],{type:"text/csv;charset=utf-8"})),a=document.createElement("a");a.href=url;a.download=`amazon-flat-file-fix-list-${fileSlug(state.fileName)}-${new Date().toISOString().slice(0,10)}.csv`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
...
els.chooseFile.addEventListener("pointerenter",()=>ensureParserLoaded().catch(()=>{}),{once:true});
els.chooseFile.addEventListener("focus",()=>ensureParserLoaded().catch(()=>{}),{once:true});
...
if(els.chooseFlatFile&&els.flatFileInput){els.chooseFlatFile.addEventListener("pointerenter",()=>ensureVariationToolsLoaded().catch(()=>{}),{once:true});els.chooseFlatFile.addEventListener("focus",()=>ensureVariationToolsLoaded().catch(()=>{}),{once:true});els.chooseFlatFile.addEventListener("click",e=>{e.stopPropagation();els.flatFileInput.click();});els.flatFileInput.addEventListener("change",e=>{const f=e.target.files?.[0];if(f)readFlatFile(f).catch(err=>toast(err.message));});}
els.loadSample.addEventListener("click",()=>{processText(sampleReport,"sample-processing-report.tsv").then(()=>trackEvent("sample_report_loaded")).catch(err=>toast(err.message));});
if(els.loadSampleFlatFile){els.loadSampleFlatFile.addEventListener("click",()=>{processVariationText(sampleFlatFile,"sample-variation-flat-file.tsv").then(()=>trackEvent("sample_flat_file_loaded")).catch(err=>toast(err.message));});}
els.clearAll.addEventListener("click",()=>{clearAll();trackEvent("results_cleared");});
els.tabButtons.forEach((b,index)=>{b.addEventListener("click",()=>{setActiveView(b.dataset.view);renderErrors();});b.addEventListener("keydown",e=>{if(!["ArrowLeft","ArrowRight","Home","End"].includes(e.key))return;e.preventDefault();const last=els.tabButtons.length-1,next=e.key==="Home"?0:e.key==="End"?last:e.key==="ArrowRight"?Math.min(index+1,last):Math.max(index-1,0);els.tabButtons[next].focus();setActiveView(els.tabButtons[next].dataset.view);renderErrors();});});
...
els.downloadCsv.addEventListener("click",()=>{downloadCsv().then(()=>trackEvent("csv_exported",{error_count:state.errors.length})).catch(err=>toast(err.message));});
document.addEventListener("click",e=>{const repairButton=e.target.closest("#downloadRepairedFlatFile");if(repairButton){ensureCsvExportLoaded().then(()=>downloadRepairedFlatFile()).catch(err=>toast(err.message));return;}const manualButton=e.target.closest("#downloadManualReviewCsv");if(manualButton){ensureCsvExportLoaded().then(()=>downloadManualReviewCsv()).catch(err=>toast(err.message));return;}const target=e.target.closest("[data-track]");if(!target)return;trackEvent(target.dataset.track,{intent:target.dataset.intent||"",source:target.dataset.source||"",href:safeTrackedHref(target.getAttribute("href")||"")});});
